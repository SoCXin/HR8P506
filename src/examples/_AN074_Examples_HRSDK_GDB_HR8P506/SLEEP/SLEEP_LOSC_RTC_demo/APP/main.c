...
*描  述:  sleep示例程序，单独使用子板进行试验(需要断开子板上电源指示灯的限流电阻)，
          当供电电压低于3.6V进行休眠程序，RTC秒唤醒并翻转PA23
...
int main(void)
{
    ...
    SCU_RegUnLock();
    SCU_LVDIFS_High(); //LVD低电压检测
    SCU_LVDVS_3V6();   //LVD休眠电压3.6V
    SCU_LVDFLT_Enable();
    SCU_LVDIT_Enable();
    SCU_LVD_Enable();
    SCU->PWRC.PORRSTF = 0;   //需要检测SOFT_RSTF标识需先将PORRSTF清零
    SCU_RegLock();
    ...
    //以下开始进行主循环初始化
    SCU_RegUnLock();
    SCU_LVDIFS_Rise(); //LVD电压降低检测
    NVIC_Init(NVIC_LVD_IRQn, NVIC_Priority_0, Enable); //LVD中断设置
    SCU_OpenXTAL();  //使能外部晶体
    SCU_RegLock();
